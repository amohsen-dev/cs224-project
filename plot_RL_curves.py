...
def generate_plot():  
  clrs = px.colors.DEFAULT_PLOTLY_COLORS
  ...
  cs224r = cs224r.join(cs224r.rolling(3, 1).mean().add(cs224r.iloc[::-1].rolling(3, 1).mean().iloc[::-1]).add_suffix(' smoothed')/2)
  ...
  fig.update_layout(title='IMP Differential of RL Agent vs BC', yaxis={'title': 'IMP Differential'}, xaxis={'title': 'Self-Play Games'})
  return fig
