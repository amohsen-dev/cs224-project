...
if __name__=='__main__':
    ...
    for i_epoch in range(num_epochs):
        ...
        pnn_output = model_pnn.forward_no_dropout(training_pnn_input).detach().numpy().argmax(axis=1)
        df = pd.DataFrame({'pred': pnn_output,  'target': training_data_y})
        pnn_output_test = model_pnn.forward_no_dropout(test_pnn_input).detach().numpy().argmax(axis=1)
        df_test = pd.DataFrame({'pred': pnn_output_test, 'target': test_data_y})
